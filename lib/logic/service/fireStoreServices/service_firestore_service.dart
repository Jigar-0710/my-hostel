...
class ServiceFirestoreService {
  ...
  Future<void> saveService(Service service) {
    return _db.collection('ServiceR').doc(service.id).set(service.createMap());
  }
  ...
  Future<void> removeService(String serviceId) {
    return _db.collection('ServiceR').doc(serviceId).delete();
  }

  Future<void> changeServiceStatus(int status, String serviceId) {
    return _db.collection('ServiceR').doc(serviceId).set(
      {'Status': status},
      SetOptions(
        merge: true,
      ),
  ...
  Future<void> changeServiceRemark(String remark, String serviceId) {
    return _db.collection('ServiceR').doc(serviceId).set(
      {'Remark': remark},
      SetOptions(
        merge: true,
